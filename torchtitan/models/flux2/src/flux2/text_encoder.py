...
class Mistral3SmallEmbedder(nn.Module):
    def __init__(
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_spec_processor, use_fast=False)
        self.processor_model_spec = model_spec_processor
        self.processor = None
        self.nsfw_classifier = None
        self.yes_token, self.no_token = self.tokenizer.encode(
    ...
    def _get_processor(self):
        if self.processor is None:
            ...
            self.processor = AutoProcessor.from_pretrained(
                self.processor_model_spec, use_fast=False
            )
    ...
    def upsample_prompt(
        ...
        try:
            ...
            raw_txt = self.tokenizer.batch_decode(
    ...
    def test_image(self, image: Image.Image | str | Path | torch.Tensor) -> bool:
        ...
        nsfw_classifier = self._get_nsfw_classifier()
        classification = next(c for c in nsfw_classifier(image) if c["label"] == "nsfw")

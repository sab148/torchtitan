...
#SBATCH --gres=gpu:4
#SBATCH -o cache/%j_%a.log # %j will be replaced by the job ID, %a by array index
#SBATCH --array=1
...
source /e/scratch/nxtaim-1/benassou1/torchtitan/sc_venv_template/activate.sh
...
# Set model and config - required by launch_jupyter.sh
export MODEL="${MODEL:-flux2}"
export CONFIG="${CONFIG:-flux2_klein_4b_dataconsolidation}"

echo " <<<< torchtitan     git commit $(git -C torchtitan rev-parse HEAD 2>/dev/null || echo 'N/A')"

srun bash launch_jupyter.sh
echo "RUNNING DONE!"